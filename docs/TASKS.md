...
**Overall progress:** `░░░░░░░░░░░░░░░░░░░░` 0% (0/34)

> Progress bar: 20 cells, one cell ≈ 1.7 tasks. Fill `█` per completed cell.

**Currently in progress:** _none_
**Last completed:** _none merged yet_
**In review:** T-01, T-02, T-03, T-04, T-05 — Sprint 0 delivered together on branch `claude/zip-folder-review-y5th0x`, awaiting human review. Counters above track `DONE` (merged); these move to `DONE` on merge.
**Next up:** T-06 (after Sprint 0 review)
...
| 7 | Keycloak realm name; are FE and BE separate clients today? | | |
...
**Status:** `REVIEW` · **Depends:** — · **Started:** 2026-07-28 · **Finished:** 2026-07-28 · **MR:** branch `claude/zip-folder-review-y5th0x`
...
**Status:** `REVIEW` · **Depends:** T-01 · **Started:** 2026-07-28 · **Finished:** 2026-07-28 · **MR:** branch `claude/zip-folder-review-y5th0x`
...
- [ ] Pipeline green — verify on first CI run after push
...
**Status:** `TODO` · **Depends:** T-01 · **Started:** — · **Finished:** — · **MR:** —

- [ ] `infra/keycloak/realm-export.json`: realm, roles `platform-admin` + `tech-user`
- [ ] Public FE client with PKCE; API client with audience `taproot-api`
- [ ] Keycloak added to `docker-compose` with the realm auto-imported
- [ ] Two test users, one per role
- [ ] Setup documented in `docs/runbook.md`
...
**Status:** `TODO` · **Depends:** T-06, T-03 · **Started:** — · **Finished:** — · **MR:** —

- [ ] JWKS fetched and cached in Redis (1h TTL, refetch on unknown `kid`)
- [ ] Validates signature, `iss`, `aud`, `exp`
- [ ] `require_role("platform-admin")` FastAPI dependency
- [ ] 401 vs 403 correctly distinguished (`ARCHITECTURE.md` §8.1)
- [ ] User upserted into `users` on first authenticated request
- [ ] Tests with locally signed fixture tokens: valid, expired, wrong audience, wrong role
